...
- **D-03: Symbol normalization stores both raw and canonical forms.** Universe Parquet has columns `ticker_raw` (iShares dot notation, e.g., `BRK.B`, `BF.B`) and `ticker` (canonical dash notation for yfinance, `BRK-B`, `BF-B`). Single `normalize_ticker()` function in `data/universe.py` applies regex `\.([A-Z])$` → `-\1` plus a tiny known-divergence allowlist for edge cases. Round-trip auditable; lossless against iShares. Downstream layers consume `ticker` only.
