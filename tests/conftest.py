...
# --- Phase 6 Wave 0 fixtures (Plan 06-01) -----------------------------------
# Path-loaders for the 7 committed fixture files generated by
# scripts/generate_phase6_fixtures.py. The 12 Phase 6 test skeletons consume
# these via the conftest dependency-injection path; Plans 06-02/06-03/06-04
# fill the test bodies and start exercising the fixtures with real assertions.


@pytest.fixture(scope="session")
def nvda_2023_vcp_panel() -> pd.DataFrame:
    """NVDA 2023-04-01..2023-07-15 OHLCV (D-02 golden file for VCP detector)."""
    return pd.read_parquet(
        Path(__file__).parent / "fixtures" / "patterns" / "nvda_2023_vcp.parquet"
    )


@pytest.fixture(scope="session")
def aapl_2020_vcp_panel() -> pd.DataFrame:
    """AAPL 2020-02-01..2020-08-31 OHLCV (D-02 COVID-recovery VCP)."""
    return pd.read_parquet(
        Path(__file__).parent / "fixtures" / "patterns" / "aapl_2020_vcp.parquet"
    )


@pytest.fixture(scope="session")
def nvda_2024_split_panel() -> pd.DataFrame:
    """NVDA 2024-04-01..2024-08-31 OHLCV (spans 2024-06-10 10:1 split; D-02/D-25)."""
    return pd.read_parquet(
        Path(__file__).parent / "fixtures" / "patterns" / "nvda_2024_split.parquet"
    )


@pytest.fixture(scope="session")
def nvda_2023_flag_panel() -> pd.DataFrame:
    """NVDA 2023-05-15..2023-06-20 OHLCV (D-02 continuation flag fixture)."""
    return pd.read_parquet(
        Path(__file__).parent / "fixtures" / "patterns" / "nvda_2023_flag.parquet"
    )


@pytest.fixture(scope="session")
def sample_quarterly_fundamentals() -> pd.DataFrame:
    """4-row FundamentalsSchema-compliant sample (AAPL/MSFT/NVDA/AMZN)."""
    return pd.read_parquet(
        Path(__file__).parent / "fixtures" / "fundamentals" / "sample_quarterly.parquet"
    )


@pytest.fixture(scope="session")
def form4_cluster_db_path() -> Path:
    """Path to the synthetic Form 4 SQLite with a 3-insider AAPL cluster."""
    return Path(__file__).parent / "fixtures" / "form4_cluster.sqlite"
